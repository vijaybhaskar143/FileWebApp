...
public class DBDataServer
{
	...
	private string AddItem(string name, int parentId, string FileType, int isDirectory, long size, byte[] content)
	{
		try
		{
			SqlConnection connection = this.GetConnection(this.connectionString);
	...
	private string AddFile(string name, int parentId, string FileType, byte[] content)
	{
		return this.AddItem(name, parentId, FileType, 0, content.LongLength, content);
	}
